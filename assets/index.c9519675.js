...
}`;let G,R,te,d,ne,k;async function we(){const t=await q();G=document.createElement("canvas"),R=G.getContext("webgpu"),te=t.adapter,d=t.device,ne=R.getPreferredFormat(te);const e=z({device:d,code:Y,fragCode:he,vertexBuffers:[{arrayStride:H,attributes:[{shaderLocation:0,offset:K,format:"float32x4"},{shaderLocation:1,offset:j,format:"float32x2"}]}]}),a=d.createSampler({magFilter:"linear",minFilter:"linear"}),o=w(d,W,GPUBufferUsage.VERTEX),n=w(d,new Float32Array(new Array(3)),GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST);k=({source:r,ratio:s,seed:p,granularity:f})=>{const{width:g,height:y}=r;G.width=g,G.height=y;const v=ue(d,g,y);d.queue.copyExternalImageToTexture({source:r},{texture:v},{width:g,height:y});const b=1,P=[G.width*b,G.height*b];R.configure({device:d,format:ne,size:P}),d.queue.writeBuffer(n,0,new Float32Array([s,p,f]));const F=d.createBindGroup({layout:e.getBindGroupLayout(0),entries:[{binding:0,resource:a},{binding:1,resource:v.createView()}]}),h=d.createBindGroup({layout:e.getBindGroupLayout(1),entries:[{binding:0,resource:{buffer:n}}]}),m=d.createCommandEncoder(),x=X(m,R);x.setPipeline(e),x.setBindGroup(0,F),x.setBindGroup(1,h),x.setVertexBuffer(0,o),x.draw(6),N(x),d.queue.submit([m?.finish()])}}async function Be(t,{value:e=0,seed:a=0,granularity:o=50}){return isNaN(e)?t:(k||await we(),k({source:t,ratio:e,seed:a,granularity:o}),G)}var Ue=`[[block]] struct Unifroms{
    sigma: f32;
