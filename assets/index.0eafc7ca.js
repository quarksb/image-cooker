...
}`;let B,R,j,f,W,_;async function pe(){const t=await k();B=document.createElement("canvas"),R=B.getContext("webgpu"),j=t.adapter,f=t.device,W=R.getPreferredFormat(j);const e=M({device:f,code:z,fragCode:me}),r=f.createSampler({magFilter:"linear",minFilter:"linear"}),a=C(f,new Float32Array(new Array(3)),GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST);_=({source:n,ratio:o,seed:c,granularity:g})=>{const{width:s,height:p}=n;B.width=s,B.height=p;const v=ee(f,s,p);f.queue.copyExternalImageToTexture({source:n},{texture:v},{width:s,height:p});const y=1,T=[B.width*y,B.height*y];R.configure({device:f,format:W,size:T}),f.queue.writeBuffer(a,0,new Float32Array([o,c,g]));const A=f.createBindGroup({layout:e.getBindGroupLayout(0),entries:[{binding:0,resource:r},{binding:1,resource:v.createView()}]}),b=f.createBindGroup({layout:e.getBindGroupLayout(1),entries:[{binding:0,resource:{buffer:a}}]}),m=f.createCommandEncoder(),x=q(m,R);x.setPipeline(e),x.setBindGroup(0,A),x.setBindGroup(1,b),x.draw(6),x.end(),f.queue.submit([m?.finish()])}}async function ve(t,{value:e=0,seed:r=0,granularity:a=50}){return isNaN(e)?t:(_||await pe(),_({source:t,ratio:e,seed:r,granularity:a}),B)}var ye=`struct Unifroms{
    sigma: f32;
...
fn frag_main(@location(0) fragUV: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    return color;
}`;let U,D,J,i,N,$;async function be(){const t=await k();U=document.createElement("canvas"),D=U.getContext("webgpu"),J=t.adapter,i=t.device,N=D.getPreferredFormat(J);const e=M({device:i,code:z,fragCode:ye}),r=i.createSampler({magFilter:"linear",minFilter:"linear"}),a=C(i,new Float32Array(new Array(4)),GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST),n=C(i,new Float32Array([1,0,0,0]),GPUBufferUsage.UNIFORM),o=C(i,new Float32Array([0,1,0,0]),GPUBufferUsage.UNIFORM),c=i.createBindGroup({layout:e.getBindGroupLayout(2),entries:[{binding:0,resource:{buffer:n}}]}),g=i.createBindGroup({layout:e.getBindGroupLayout(2),entries:[{binding:0,resource:{buffer:o}}]});$=({source:s,blur:p})=>{const{width:v,height:y}=s;U.width=v,U.height=y;const T=1,A=[U.width*T,U.height*T],b={width:v,height:y},m=i.createTexture({size:b,format:N,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC|GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING}),x=i.createTexture({size:b,format:N,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC|GPUTextureUsage.TEXTURE_BINDING});i.queue.copyExternalImageToTexture({source:s},{texture:m},b),D.configure({device:i,format:N,size:A});const ie={colorAttachments:[{view:x.createView(),loadOp:"load",storeOp:"store"}]};i.queue.writeBuffer(a,0,new Float32Array([p,0,v,y]));const ue=i.createBindGroup({layout:e.getBindGroupLayout(0),entries:[{binding:0,resource:r},{binding:1,resource:m.createView()}]}),se=i.createBindGroup({layout:e.getBindGroupLayout(0),entries:[{binding:0,resource:r},{binding:1,resource:x.createView()}]}),X=i.createBindGroup({layout:e.getBindGroupLayout(1),entries:[{binding:0,resource:{buffer:a}}]}),O=i.createCommandEncoder();let l=O.beginRenderPass(ie);l.setPipeline(e),l.setBindGroup(0,ue),l.setBindGroup(1,X),l.setBindGroup(2,c),l.draw(6),l.end(),l=q(O,D),l.setPipeline(e),l.setBindGroup(0,se),l.setBindGroup(1,X),l.setBindGroup(2,g),l.draw(6),l.end(),i.queue.submit([O.finish()])}}async function xe(t,{value:e=0}){return isNaN(e)?t:($||await be(),$({source:t,blur:e}),U)}const Q=["\u81EA\u5F3A\u4E0D\u606F","\u539A\u5FB7\u8F7D\u7269","\u7CBE\u76CA\u6C42\u7CBE","\u4E0A\u5584\u82E5\u6C34"];async function we(t){if(!t)return;const e=document.createElement("a");t.toBlob(r=>{const a=URL.createObjectURL(r);e.download=`\u5938\u514B${Q[Math.floor(Math.random()*Q.length)]}.png`,e.href=a,e.click()})}function he(t){return new Promise(e=>{const r=new FileReader;r.readAsDataURL(t),r.onload=a=>{const n=a?.target?.result;e({url:n})}})}const Be={blur:xe,noise:ve,warp:ge},E=document.getElementById("canvas");E.style.maxWidth="1200px";const Ue=1200,Ge=675;E.width=Ue;E.height=Ge;const u={blur:40,warp:100,seed:0,noise:40,granularity:10,center:{x:0,y:0},backgroundColor:"#88ddff"},S=new ce.exports.Pane,te=[],Te=S.addFolder({title:"\u80CC\u666F"});te.push(Te.addInput(u,"backgroundColor",{label:"\u80CC\u666F\u8272",view:"color"}));const G=[],Y=S.addFolder({title:"\u6D88\u878D"}),Ee=S.addFolder({title:"\u6A21\u7CCA"}),ne=S.addFolder({title:"\u626D\u66F2"});G.push(Y.addInput(u,"noise",{label:"\u6D88\u878D\u5F3A\u5EA6",min:0,max:100}));G.push(Y.addInput(u,"granularity",{label:"\u6D88\u878D\u7C92\u5EA6",min:0,max:100}));G.push(Y.addInput(u,"seed",{label:"\u6D88\u878D\u79CD\u5B50",min:0,max:1}));G.push(Ee.addInput(u,"blur",{label:"\u6A21\u7CCA\u5F3A\u5EA6",min:0,max:200}));G.push(ne.addInput(u,"warp",{label:"\u626D\u66F2\u5F3A\u5EA6",min:-100,max:100}));G.push(ne.addInput(u,"center",{label:"\u626D\u66F2\u4E2D\u5FC3",picker:"inline",expanded:!0,x:{step:1,min:-100,max:100},y:{step:1,min:-100,max:100}}));const Se=S.addButton({title:"\u4E0A\u4F20\u56FE\u7247"}),Ae=S.addButton({title:"\u4E0B\u8F7D\u56FE\u7247"}),Z=E.getContext("2d");let L,re="https://st0.dancf.com/gaoding-material/0/images/223463/20191107-203725-leuLE.jpg";const w=document.createElement("input");w.type="file";w.accept="image/png";w.style.display="none";w.addEventListener("change",async()=>{if(w.files.length){const{url:t}=await he(w.files[0]);re=t,oe()}});document.body.appendChild(w);Se.on("click",()=>{w.click()});Ae.on("click",()=>{console.log(111),we(canvas)});oe();async function ae(){const{width:t,height:e}=L;E.width=t,E.height=e;const r=[{type:"noise",enable:u.noise>0,params:{value:100-u.noise,seed:u.seed,granularity:u.granularity}},{type:"blur",enable:u.blur>0,params:{value:u.blur,k:0}},{type:"warp",enable:u.warp>0||u.warp<0,params:{value:u.warp,center:u.center}}];let a=L;for(let n=0;n<r.length;n++){const{type:o,enable:c,params:g}=r[n];if(console.time(o),c){const s=Be[o];s&&(a=await s(a,g))}console.timeEnd(o)}Z.clearRect(0,0,t,e),Z.drawImage(a,0,0)}async function oe(){const e=await(await fetch(re)).blob();L=await createImageBitmap(e),ae()}const Ce=document.querySelector("body");te.forEach(t=>{t.on("change",()=>{Ce.style.backgroundColor=u.backgroundColor})});G.forEach(t=>{t.on("change",()=>{ae()})});
